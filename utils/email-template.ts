...
export const emailTemplates = [
  {
    ...
    generateSubject: (data: TemplateData) =>
      `📅 Reminder: Your ${data.subscriptionName} Subscription Renews in 7 Days!`,
    generateBody: (data: TemplateData) => generateEmailTemplate({ ...data, daysLeft: 7 }),
  },
  {
    label: '5 days before reminder',
    generateSubject: (data: TemplateData) =>
      `⏳ ${data.subscriptionName} Renews in 5 Days – Stay Subscribed!`,
    generateBody: (data: TemplateData) => generateEmailTemplate({ ...data, daysLeft: 5 }),
  },
  {
    label: '2 days before reminder',
    generateSubject: (data: TemplateData) =>
      `🚀 2 Days Left!  ${data.subscriptionName} Subscription Renewal`,
    generateBody: (data: TemplateData) => generateEmailTemplate({ ...data, daysLeft: 2 }),
  },
  {
    label: '1 days before reminder',
    generateSubject: (data: TemplateData) =>
      `⚡ Final Reminder: ${data.subscriptionName} Renews Tomorrow!`,
    generateBody: (data: TemplateData) => generateEmailTemplate({ ...data, daysLeft: 1 }),
  },
];
